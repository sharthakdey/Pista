  ...
  if (!weakTopics.length) {
    return (
      <EmptyState
        ...
        message="Every topic is above 65%. Keep your streak going with a mixed quiz."
      />
    );
  }
    ...
    <div className="grid gap-4 md:grid-cols-3">
      {GROUPS.map((g) => {
        ...
        return (
          <div
            key={g.key}
            className="card flex max-h-[440px] min-h-[240px] flex-col p-5"
          >
            <h3 className="flex items-center gap-2 text-base font-bold">
              <span aria-hidden>{g.emoji}</span>
              {g.title}

              <span className="ml-auto rounded-full bg-surface px-2 py-0.5 font-sans text-xs font-semibold text-muted">
                {items.length}
              </span>
            </h3>
            ...
            {items.length === 0 ? (
              ...
              <>
                <ul className="mt-4 min-h-0 flex-1 space-y-3 overflow-y-auto pr-1 scrollbar-thin">
                  {items.map((w) => (
                    <li
                      key={`${w.subjectId || w.subject}-${w.topic}`}
                    ...
                    >
                      <div className="flex items-start justify-between gap-2">
                        <div className="min-w-0">
                          ...
                          <p className="text-xs text-muted">
                            {w.subject || "General"}
                            {w.examInDays != null
                              ? `, exam in ${w.examInDays} days`
                              : ""}
                          </p>
                        </div>

                        <span
                          className={`font-display text-2xl font-bold leading-none ${meta.text}`}
                        >
                          {w.mastery}%
                        </span>
                      </div>
                      ...
                      <div className="mt-3 flex gap-2">
                        ...
                        <Button
                          ...
                          variant="secondary"
                          icon={BookOpenCheck}
                          className="flex-1 hover:-translate-y-0.5"
                          onClick={() =>
                            navigate("/tutor", {
                              state: {
                                prompt: `Teach me ${w.topic} from ${w.subject}. This is a weak topic for me, so explain it step by step with simple examples and focus on helping me improve.`,
                              },
                            })
                          }
                        >
                ...
                {canScroll && (
                  <p className="mt-2 shrink-0 text-center text-[11px] font-medium text-muted">
                    Scroll to see all {items.length} topics
                  </p>
                )}
              </>
            )}
      ...
      {loading ? (
        ...
        <>
          <div className="grid gap-5 lg:grid-cols-[340px_1fr]">
            ...
            <section
              className="card p-6"
              aria-labelledby="subject-progress"
            >
              <h2
                id="subject-progress"
