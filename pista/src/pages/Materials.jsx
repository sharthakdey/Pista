...
function TypeIcon({ type }) {
  ...
  const Icon =
    t.startsWith("ppt") ? Presentation : t === "pdf" ? FileText : File;
...
function StatusChip({ status }) {
  if (status === "processed") {
    return (
      <span className="inline-flex items-center gap-1.5 rounded-full bg-mint-50 px-2.5 py-1 text-xs font-semibold text-mint-600">
...
function Searchable({ yes }) {
  return (
    ...
    >
      {yes ? "Ready for questions" : "Processing"}
    </span>
  );
}
...
export default function Materials() {
  ...
  const pollWhile = useMemo(
    () => (list) =>
      Array.isArray(list) &&
      list.some((m) => m.status === "processing"),
    []
  );

  const materials = useApi(getMaterials, [], {
    pollMs: 4000,
    pollWhile,
  });
  ...
  const startUploads = async (fileList) => {
    ...
    for (const item of items) {
      try {
        ...
        notify(
          `${item.file.name} uploaded. PISTA is processing it now.`,
          "success"
        );
  ...
  const onDelete = async (materialId, fileName) => {
    if (
      !window.confirm(
        `Are you sure you want to delete "${fileName}"? PISTA will forget its contents.`
      )
    ) {
      return;
    }
  ...
  const list = (materials.data || []).filter(
    (m) =>
      (filter === "all" ||
        (m.subjectId || m.subject) === filter) &&
      (!query ||
        m.fileName
          .toLowerCase()
  ...
  return (
    <>
      <PageHeader
        ...
        subtitle="Give PISTA the knowledge you learn from."
      />
        ...
        <div
          ...
          className={`relative flex flex-col items-center rounded-2xl border-2 border-dashed px-6 py-10 text-center transition sm:py-12 ${
            dragging
              ...
              : "border-ink-300/60 bg-surface/60"
          }`}
        >
          <span
            className={`grid h-16 w-16 place-items-center rounded-3xl bg-white shadow-card transition ${
              dragging ? "scale-110" : ""
            }`}
          >
            <UploadCloud
          ...
          <p className="mt-1 text-sm font-medium text-muted">
            PDF • PPT • PPTX • DOC • DOCX • TXT, up to 50 MB
          </p>
        ...
        {uploads.length > 0 && (
          ...
          >
            {uploads.map((u) => (
              <li
                key={u.id}
                className="flex animate-fade-in items-center gap-3 rounded-2xl border border-line p-3"
              >
                <TypeIcon
                  type={u.file.name.split(".").pop()}
                />
                ...
                {u.status === "error" && (
                  <button
                    ...
                    className="text-sm font-semibold text-muted hover:text-ink-900"
                  >
                    Dismiss
                  </button>
                )}
      ...
      >
        ...
        {subjects.length > 0 && (
          ...
          >
            {[{ id: "all", name: "All subjects" }, ...subjects].map(
              (s) => (
                <button
                  key={s.id}
                  role="tab"
                  ...
                  className={`inline-flex shrink-0 items-center gap-2 rounded-full px-3.5 py-1.5 text-sm font-semibold transition ${
                    filter === s.id
                      ...
                      : "bg-white text-ink-700 ring-1 ring-line hover:ring-brand-200"
                  }`}
                >
                  {s.id !== "all" && (
                    <SubjectDot subject={s.id} />
                  )}
                  {s.name}
                </button>
              )
        ...
        {materials.error ? (
          <ErrorState
            ...
            title="Your materials didn't load"
          />
        ) : materials.loading ? (
          ...
          <EmptyState
            ...
            message="Try another subject or search term."
          />
        ) : (
          <>
                    ...
                    >
                      <td className="px-5 py-3.5">
                        <div className="flex items-center gap-3">
                          ...
                          <div className="min-w-0">
                            <p
                              className="max-w-[260px] truncate font-semibold lg:max-w-[340px]"
                              title={m.fileName}
                            >
                              {m.fileName}
                            </p>
                      ...
                      <td className="px-5 py-3.5 text-right">
                        <button
                          ...
                          className="rounded-lg p-2 text-ink-400 transition hover:bg-coral-50 hover:text-coral-600"
