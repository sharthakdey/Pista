...
function Milestone({ className, icon: Icon, label, tone, delay }) {
  return (
    <div className={`absolute hidden animate-pop-in items-center gap-2 rounded-full bg-ink-800/90 py-1.5 pl-1.5 pr-3 text-xs font-semibold text-white ring-1 ring-white/10 backdrop-blur lg:flex ${className}`}
...
function ProductPreview() {
  return (
    <div className="relative mx-auto w-full max-w-[560px]" aria-label="Preview of the PISTA dashboard and AI tutor" role="img">
      <div className="overflow-hidden rounded-[28px] bg-white shadow-[0_40px_80px_-30px_rgba(0,0,0,.6)] ring-1 ring-white/10">
        <div className="flex">
          ...
          <div className="min-w-0 flex-1 bg-surface p-4 pb-14">
            <p className="font-display text-base font-bold text-ink-900">Good morning, Harman 👋</p>
            ...
            <div className="mt-3 grid grid-cols-3 gap-2">
              {[["4 days", "CN exam"], ["72%", "Last quiz"], ["🔥 6", "Day streak"]].map(([v, l]) => (
                <div key={l} className="rounded-xl bg-white p-2.5 ring-1 ring-line">
      ...
      <div className="relative -mt-10 ml-auto w-[88%] animate-fade-in rounded-2xl bg-white p-4 shadow-lift ring-1 ring-line sm:-mr-6" style={{ animationDelay: ".9s" }}>
        <div className="flex items-start gap-2.5">
          <PistaOrb size={28} />
          <div className="min-w-0 text-[13px] leading-relaxed text-ink-800">
            <p><span className="font-semibold text-ink-900">Go-Back-N</span> lets the sender transmit up to <span className="font-semibold">W</span> frames. If frame 3 is lost, it resends 3, 4 and 5.</p>
            <span className="mt-2 inline-flex items-center gap-1 rounded-full bg-electric-500/10 px-2 py-0.5 text-[11px] font-semibold text-electric-600">📚 Course Material</span>
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="min-h-dvh bg-surface">
      ...
      <div className="relative overflow-hidden bg-ink-900 text-white">
        <div className="pointer-events-none absolute -left-40 top-20 h-[420px] w-[420px] rounded-full bg-electric-500/15 blur-[100px]" aria-hidden />
        <div className="pointer-events-none absolute -right-20 -top-20 h-[460px] w-[460px] rounded-full bg-violet-500/20 blur-[110px]" aria-hidden />
        ...
        <header className="relative z-10 mx-auto flex h-20 max-w-[1200px] items-center justify-between px-5 sm:px-8">
          ...
          <nav className="flex items-center gap-1 sm:gap-2" aria-label="Site">
            <a href="#features" className="hidden rounded-xl px-3 py-2 text-sm font-semibold text-ink-300 hover:text-white sm:block">Features</a>
            <a href="#how" className="hidden rounded-xl px-3 py-2 text-sm font-semibold text-ink-300 hover:text-white sm:block">How it works</a>
            <Button to="/dashboard" variant="glass" size="sm">Open app</Button>
          </nav>
        </header>

        <section className="relative mx-auto max-w-[1200px] px-5 pb-20 pt-6 sm:px-8 lg:pb-28 lg:pt-10">
          <div className="absolute inset-x-0 top-0 h-[520px]"><HeroPath /></div>
          <Milestone className="left-[28%] top-[250px]" icon={FileSearch} label="Lecture 5 notes" tone="bg-electric-500" delay=".7s" />
          <Milestone className="left-[52%] top-[330px]" icon={CalendarClock} label="Exam in 4 days" tone="bg-amber-500" delay="1.1s" />
          <Milestone className="right-[8%] top-[48px]" icon={Target} label="ARQ Protocols" tone="bg-violet-500" delay="1.6s" />

          <div className="relative grid items-center gap-12 lg:grid-cols-[1.05fr_1fr]">
            <div>
              <p className="font-display text-[88px] font-extrabold leading-[.85] tracking-[-0.05em] sm:text-[132px] lg:text-[168px]">
                <span className="bg-gradient-to-br from-white via-white to-brand-200 bg-clip-text text-transparent">PISTA</span>
              </p>
              <h1 className="mt-6 max-w-xl text-[34px] font-bold leading-[1.08] sm:text-[46px]">
                Your Personal AI Study Path.
              </h1>
              <p className="mt-5 max-w-lg text-[17px] leading-relaxed text-ink-300">
                PISTA combines your courses, exams, learning progress and study material into one intelligent AI tutor.
              </p>
              ...
              <p className="mt-6 text-sm text-ink-400">Study smarter. Track progress. Master your exams.</p>
            </div>
            <ProductPreview />
          </div>
        </section>
      ...
      {/* Features */}
      <section id="features" className="mx-auto max-w-[1200px] scroll-mt-4 px-5 py-20 sm:px-8 lg:py-28">
        <div className="max-w-2xl">
          <h2 className="text-[34px] font-bold leading-tight sm:text-[44px]">One tutor that knows your whole semester</h2>
        ...
        <div className="mt-12 grid gap-4 md:grid-cols-6">
          ...
          <article className="relative overflow-hidden rounded-3xl bg-ink-900 p-7 text-white md:col-span-4 md:row-span-2">
            ...
            <span className="grid h-12 w-12 place-items-center rounded-2xl bg-white/10"><Sparkles className="h-6 w-6 text-violet-400" aria-hidden /></span>
          ...
          {FEATURES.slice(1).map(({ icon: Icon, title, text }, i) => (
            <article key={title} className={`card p-6 ${i < 2 ? "md:col-span-2" : "md:col-span-3"}`}>
              <span className="grid h-11 w-11 place-items-center rounded-2xl bg-brand-50 text-brand-600"><Icon className="h-5 w-5" aria-hidden /></span>
      ...
      <section id="how" className="border-y border-line bg-white">
        <div className="mx-auto max-w-[1200px] px-5 py-20 sm:px-8">
          <h2 className="text-[30px] font-bold leading-tight sm:text-[38px]">How PISTA builds your path</h2>
          <ol className="relative mt-10 grid gap-8 md:grid-cols-4 md:gap-6">
            <span className="absolute left-6 right-6 top-6 hidden h-0.5 bg-gradient-to-r from-electric-500 to-violet-500 md:block" aria-hidden />
            {STEPS.map(({ icon: Icon, title, text }, i) => (
              <li key={title} className="relative flex gap-4 md:block">
                <span className="relative grid h-12 w-12 shrink-0 place-items-center rounded-full bg-ink-900 text-white ring-4 ring-white">
                  <Icon className="h-5 w-5" aria-hidden />
                </span>
                <div>
                  <p className="text-sm font-semibold text-brand-600 md:mt-5">Step {i + 1}</p>
      ...
      <section className="mx-auto max-w-[1200px] px-5 py-20 sm:px-8">
        <div className="relative overflow-hidden rounded-[32px] bg-gradient-to-br from-brand-700 via-brand-600 to-violet-600 px-6 py-14 text-center text-white sm:px-12">
          <h2 className="mx-auto max-w-2xl text-[32px] font-bold leading-tight sm:text-[42px]">Know exactly what to study next</h2>
          <p className="mx-auto mt-3 max-w-lg text-white/80">Open your dashboard and see today's recommendation.</p>
          <Button to="/dashboard" variant="light" size="lg" className="mt-8" iconRight={ArrowRight}>Start Studying</Button>
        </div>
      </section>

      <footer className="mx-auto flex max-w-[1200px] flex-col items-center justify-between gap-3 px-5 pb-10 text-sm text-muted sm:flex-row sm:px-8">
        <Logo light={false} size={30} />
        <p>PISTA, your personal AI study path.</p>
        <Link to="/settings" className="font-semibold hover:text-ink-900">Settings</Link>
      </footer>
    </div>
  );
}
