...
def _pretty(raw):
    ...
    for w in s.split():
        if not any(v in w.lower() for v in "aeiou") and len(w) <= 6:
            out.append(w.upper())          # acronyms like dbms, ofdm
        else:
...
def student_me(x_student_id: Optional[str] = Header(None)):
    ...
    return {
        ...
        "stats": {                                   # <-- unsticks "Loading profile…"
...
def progress_me(x_student_id: Optional[str] = Header(None)):
    ...
    # Real quiz history from the activity container
    try:
        quiz_docs = list(activity.query_items(
            query="SELECT * FROM c WHERE c.student_id = @sid",
            parameters=[{"name": "@sid", "value": student_id}],
    ...
    # Topics built from progress entries (fixes "Topics 0")
    topics = [{
    ...
    # Subjects: prefer stored list, otherwise derive from topics + quizzes
    subj_map = {}
    ...
    # Weak topics enriched with subject NAME + days-to-exam
    weak_topics = []
    for w in raw_weak:
...
async def create_quiz(
    ...
    # 1. Extract text if a file was uploaded
...
Generate exactly {numQuestions} questions. 'correctOption' MUST be the 0-based index of the correct answer in the 'options' array."""
    ...
    try:
        ...
        # Store the full quiz (with answers) in memory for grading later
        ACTIVE_QUIZZES[quiz_data["quizId"]] = quiz_data
        
        # Create a "safe" version to send to the frontend (hide the answers!)
...
def submit_quiz(quiz_id: str, payload: dict, x_student_id: Optional[str] = Header(None)):
    ...
    # 1. Grade the quiz & track topic performance
    review = []
    correct_count = 0
    topic_stats = {} # Tracks correct/total per subtopic
    ...
    # Build the exact arrays the frontend QuizResults.jsx expects
    topic_performance = []
    weak_areas = []
    ...
    # 2. Update Cosmos DB Progress & Weak Topics
    try:
    ...
    return {
        ...
        "topicPerformance": topic_performance,  # <--- THIS WAS MISSING!
        "weakAreas": weak_areas,                # <--- NOW PROPERLY FORMATTED
...
def guide_chat(req: ChatRequest, x_student_id: Optional[str] = Header(None)):
    """Dedicated chat for the Guide Agent."""
    student_id = x_student_id or "student-001"
    
    context_doc = T.get_student_context(students, student_id)
    exams = T.get_upcoming_exams(students, student_id)
...
--------------------------------"""
    ...
    input_messages = [{"role": "system", "content": snapshot}]
...
def recommendation_today(x_student_id: Optional[str] = Header(None)):
    """Automatically asks the Guide for a daily study plan."""
    student_id = x_student_id or "student-001"
...
def list_exams(x_student_id: Optional[str] = Header(None)):
    ...
    for e in doc.get("exams", []) or []:
        ...
        if prep is None:  # compute from real quiz mastery when possible
...
async def extract_exams(
    ...
    """Upload a datesheet PDF -> AI extracts exam rows -> saved to Cosmos DB."""
    student_id = x_student_id or "student-001"
    content = await file.read()
    if not (file.filename or "").lower().endswith(".pdf"):
        raise HTTPException(status_code=400, detail="Please upload the datesheet as a PDF.")
    try:
        from pypdf import PdfReader
        reader = PdfReader(io.BytesIO(content))
